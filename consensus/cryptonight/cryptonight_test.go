...
func hashVariant1(input []byte) []byte {
	return hashCryptonight(input, 1)
}

func hashVariant2(input []byte) []byte {
	return hashCryptonight(input, 2)
}
...
func TestHashVariant2ForEthereum(t *testing.T) {
	...
	expected_hash := hexutil.MustDecode("0xa0e217e26c0c5c409a9e7119a8a7b1c4faa5886a2c101116548ab323f11bc4c2")
	if !bytes.Equal(result, expected_hash) {
		t.Error("Unexpected result: ", hex.EncodeToString(result), " versus ", hex.EncodeToString(expected_hash))
	}
}
