...
Building Swarm requires Go (version 1.10 or later).
    ...
    go get -d gitlab.neji.vm.tc/marconi/go-ethereum

    go install gitlab.neji.vm.tc/marconi/go-ethereum/cmd/swarm
...
You must have your working copy under `$GOPATH/src/gitlab.neji.vm.tc/marconi/go-ethereum`.
...
```
git clone git@github.com:nirname/go-ethereum.git $GOPATH/src/gitlab.neji.vm.tc/marconi/go-ethereum
```
...
```
stateth --rm --grafana-dashboards-folder $GOPATH/src/gitlab.neji.vm.tc/marconi/go-ethereum/swarm/grafana_dashboards --influxdb-database metrics
```
 ...
 * [Code review guidelines](https://gitlab.neji.vm.tc/marconi/go-ethereum/wiki/Code-Review-Guidelines).
